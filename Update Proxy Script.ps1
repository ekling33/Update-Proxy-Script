...
# IIS is intentionally stopped before requesting the new certificate thumbprint.
# Once stopped, the finally block ensures IIS is started again even if the script errors.
$IisStopped = $false
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "iisreset /stop failed with exit code $LASTEXITCODE."
    }

    $IisStopped = $true

    do {
        $Thumbprint = Read-Host 'Enter the new certificate thumbprint from Local Computer\Personal'
    ...
    if ($Confirmation -cne 'YES') {
        Write-Host 'No certificate bindings were changed.' -ForegroundColor Yellow
        return
    }
    ...
    if ($IisStopped) {
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Warning "iisreset /start failed with exit code $LASTEXITCODE. IIS may need to be started manually."
        }
    }
}
